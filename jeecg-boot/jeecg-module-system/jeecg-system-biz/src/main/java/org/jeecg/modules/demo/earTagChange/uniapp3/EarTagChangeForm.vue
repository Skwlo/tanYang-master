...
navigationBarTitleText: '耳号更换记录表',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['oldCommonEarTag']"
                  :label="get4Label('旧普通耳号')"
                  name='oldCommonEarTag'
                  prop='oldCommonEarTag'
                  placeholder="请选择旧普通耳号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['newCommonEarTag']"
                  :label="get4Label('新普通耳号')"
                  name='newCommonEarTag'
                  prop='newCommonEarTag'
                  placeholder="请选择新普通耳号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['oldElectronicEarTag']"
                  :label="get4Label('旧电子耳号')"
                  name='oldElectronicEarTag'
                  prop='oldElectronicEarTag'
                  placeholder="请选择旧电子耳号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['newElectronicEarTag']"
                  :label="get4Label('新电子耳号')"
                  name='newElectronicEarTag'
                  prop='newElectronicEarTag'
                  placeholder="请选择新电子耳号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('更换日期')"
                  labelWidth="100px"
                  type="date"
                  name='changeDate'
                  v-model:value="myFormData['changeDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('EarTagChangeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/earTagChange/earTagChange/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/earTagChange/earTagChange/edit':'/earTagChange/earTagChange/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
