...
<template>
  <PageLayout navTitle="出库记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './StockOutData';
defineOptions({
  name: 'StockOutList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/stockOut/stockOut/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/stockOut/stockOut/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
