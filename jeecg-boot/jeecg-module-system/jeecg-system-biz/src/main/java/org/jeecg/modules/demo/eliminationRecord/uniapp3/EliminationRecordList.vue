...
<template>
  <PageLayout navTitle="淘汰记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './EliminationRecordData';
defineOptions({
  name: 'EliminationRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/eliminationRecord/eliminationRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/eliminationRecord/eliminationRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
