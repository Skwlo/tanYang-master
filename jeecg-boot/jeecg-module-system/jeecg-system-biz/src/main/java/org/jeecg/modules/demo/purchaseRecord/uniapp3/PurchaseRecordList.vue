...
<template>
  <PageLayout navTitle="采购记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './PurchaseRecordData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/purchaseRecord/purchaseRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/purchaseRecord/purchaseRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
