...
<template>
  <PageLayout navTitle="经销商表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './DealerData';
defineOptions({
  name: 'DealerList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/dealer/dealer/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/dealer/dealer/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
