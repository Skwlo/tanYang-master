...
<template>
  <PageLayout navTitle="销售明细表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './SaleRecordDetailData';
defineOptions({
  name: 'SaleRecordDetailList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/saleRecordDetail/saleRecordDetail/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/saleRecordDetail/saleRecordDetail/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
