...
<template>
  <PageLayout navTitle="库存表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './InventoryData';
defineOptions({
  name: 'InventoryList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/inventory/inventory/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/inventory/inventory/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
