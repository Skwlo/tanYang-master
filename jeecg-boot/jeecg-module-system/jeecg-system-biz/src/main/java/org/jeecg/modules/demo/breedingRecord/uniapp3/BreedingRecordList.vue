...
<template>
  <PageLayout navTitle="配种记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BreedingRecordData';
defineOptions({
  name: 'BreedingRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/breedingRecord/breedingRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/breedingRecord/breedingRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
