...
<template>
  <PageLayout navTitle="死亡记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './DeathRecordData';
defineOptions({
  name: 'DeathRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/deathRecord/deathRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/deathRecord/deathRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
