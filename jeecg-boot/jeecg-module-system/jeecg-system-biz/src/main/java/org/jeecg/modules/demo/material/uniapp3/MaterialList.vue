...
<template>
  <PageLayout navTitle="物资表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './MaterialData';
defineOptions({
  name: 'MaterialList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/material/material/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/material/material/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
