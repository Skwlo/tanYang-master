...
<template>
  <PageLayout navTitle="销售记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './SaleRecordData';
defineOptions({
  name: 'SaleRecordList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/saleRecord/saleRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/saleRecord/saleRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
