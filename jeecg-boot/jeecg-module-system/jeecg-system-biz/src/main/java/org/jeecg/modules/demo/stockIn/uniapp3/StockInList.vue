...
<template>
  <PageLayout navTitle="入库记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './StockInData';
defineOptions({
  name: 'StockInList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/stockIn/stockIn/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/stockIn/stockIn/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
