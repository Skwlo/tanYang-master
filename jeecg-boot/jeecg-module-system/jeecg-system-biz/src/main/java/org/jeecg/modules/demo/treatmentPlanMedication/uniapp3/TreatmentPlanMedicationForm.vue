...
navigationBarTitleText: '治疗方案用药明细表',
},
}
</route>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('药物 ID')"
                  name='materialId'
                  prop='materialId'
                  placeholder="请选择药物 ID"
                  :rules="[
...
defineOptions({
  name: 'TreatmentPlanMedicationForm',
  options: {
...
const dataId = ref('')
const backRouteName = ref('TreatmentPlanMedicationList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/treatmentPlanMedication/treatmentPlanMedication/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/treatmentPlanMedication/treatmentPlanMedication/edit':'/treatmentPlanMedication/treatmentPlanMedication/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
