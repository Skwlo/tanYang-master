...
<template>
  <PageLayout navTitle="畜只表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LivestockData';
defineOptions({
  name: 'LivestockList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/livestock/livestock/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/livestock/livestock/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
