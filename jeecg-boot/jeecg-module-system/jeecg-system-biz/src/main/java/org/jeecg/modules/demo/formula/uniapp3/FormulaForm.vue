      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('适用阶段（幼崽 / 育肥 / 成年）')"
                 labelWidth="100px"
                 type="list"
                 name='applicableStage'
                 dict=""
                v-model="myFormData['applicableStage']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('FormulaList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/formula/formula/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/formula/formula/edit':'/formula/formula/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
