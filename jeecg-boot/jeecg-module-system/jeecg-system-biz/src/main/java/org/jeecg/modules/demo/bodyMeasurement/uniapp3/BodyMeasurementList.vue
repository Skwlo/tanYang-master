...
<template>
  <PageLayout navTitle="体尺测量记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './BodyMeasurementData';
defineOptions({
  name: 'BodyMeasurementList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/bodyMeasurement/bodyMeasurement/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/bodyMeasurement/bodyMeasurement/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
