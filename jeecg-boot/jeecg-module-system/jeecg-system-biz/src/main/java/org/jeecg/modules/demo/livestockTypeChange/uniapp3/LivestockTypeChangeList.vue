...
<template>
  <PageLayout navTitle="畜只类型变更记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LivestockTypeChangeData';
defineOptions({
  name: 'LivestockTypeChangeList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/livestockTypeChange/livestockTypeChange/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/livestockTypeChange/livestockTypeChange/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
