...
<template>
  <PageLayout navTitle="配方原料表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './FormulaIngredientData';
defineOptions({
  name: 'FormulaIngredientList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/formulaIngredient/formulaIngredient/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/formulaIngredient/formulaIngredient/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
