...
<template>
  <PageLayout navTitle="耳号更换记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './EarTagChangeData';
defineOptions({
  name: 'EarTagChangeList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/earTagChange/earTagChange/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/earTagChange/earTagChange/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
