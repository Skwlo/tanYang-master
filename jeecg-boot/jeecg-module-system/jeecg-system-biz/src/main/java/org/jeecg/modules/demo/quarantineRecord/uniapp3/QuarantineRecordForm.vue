        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('畜只 ID（可选，群体检疫时为空）')"
                  name='livestockId'
                  prop='livestockId'
                  placeholder="请选择畜只 ID（可选，群体检疫时为空）"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['shedPenId']"
                  :label="get4Label('棚栏 ID（群体检疫时使用）')"
                  name='shedPenId'
                  prop='shedPenId'
                  placeholder="请选择棚栏 ID（群体检疫时使用）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('检疫日期')"
                  labelWidth="100px"
                  type="date"
                  name='quarantineDate'
                  v-model:value="myFormData['quarantineDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['item']"
                  :label="get4Label('检疫项目')"
                  name='item'
                  prop='item'
                  placeholder="请选择检疫项目"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['result']"
                  :label="get4Label('检疫结果')"
                  name='result'
                  prop='result'
                  placeholder="请选择检疫结果"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['reportPath']"
                  :label="get4Label('检疫报告文件路径')"
                  name='reportPath'
                  prop='reportPath'
                  placeholder="请选择检疫报告文件路径"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('QuarantineRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/quarantineRecord/quarantineRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/quarantineRecord/quarantineRecord/edit':'/quarantineRecord/quarantineRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
