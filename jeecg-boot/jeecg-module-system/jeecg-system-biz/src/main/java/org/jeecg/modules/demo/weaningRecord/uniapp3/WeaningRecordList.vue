...
<template>
  <PageLayout navTitle="断奶记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './WeaningRecordData';
defineOptions({
  name: 'WeaningRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/weaningRecord/weaningRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/weaningRecord/weaningRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
