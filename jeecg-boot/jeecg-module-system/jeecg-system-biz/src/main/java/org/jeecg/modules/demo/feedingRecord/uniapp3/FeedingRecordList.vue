...
<template>
  <PageLayout navTitle="喂食记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './FeedingRecordData';
defineOptions({
  name: 'FeedingRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/feedingRecord/feedingRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/feedingRecord/feedingRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
