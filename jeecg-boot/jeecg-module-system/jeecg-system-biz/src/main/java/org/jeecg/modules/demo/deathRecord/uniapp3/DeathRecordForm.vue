      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('死亡日期')"
                  labelWidth="100px"
                  type="date"
                  name='deathDate'
                  v-model:value="myFormData['deathDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['disposalMethod']"
                  :label="get4Label('处理方式')"
                  name='disposalMethod'
                  prop='disposalMethod'
                  placeholder="请选择处理方式"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('DeathRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/deathRecord/deathRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/deathRecord/deathRecord/edit':'/deathRecord/deathRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
