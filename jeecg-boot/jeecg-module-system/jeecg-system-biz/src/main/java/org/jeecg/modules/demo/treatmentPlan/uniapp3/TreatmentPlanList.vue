...
<template>
  <PageLayout navTitle="疾病治疗方案表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TreatmentPlanData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/treatmentPlan/treatmentPlan/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/treatmentPlan/treatmentPlan/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
