...
<template>
  <PageLayout navTitle="免疫记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './VaccinationRecordData';
defineOptions({
  name: 'VaccinationRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/vaccinationRecord/vaccinationRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/vaccinationRecord/vaccinationRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
