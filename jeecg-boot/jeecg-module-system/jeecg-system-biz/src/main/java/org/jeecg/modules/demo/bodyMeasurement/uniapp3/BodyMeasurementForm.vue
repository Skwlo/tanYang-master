...
navigationBarTitleText: '体尺测量记录表',
},
}
</route>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('测量日期')"
                  labelWidth="100px"
                  type="date"
                  name='measureDate'
                  v-model:value="myFormData['measureDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('测量阶段（羔羊期 / 育肥期等）')"
                 ...
                 name='measureStage'
                 dict=""
                v-model="myFormData['measureStage']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['bodyHeight']"
                   :label="get4Label('体高 (cm)')"
                   name='bodyHeight'
                   prop='bodyHeight'
                   placeholder="请选择体高 (cm)"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['bodyWeight']"
                   :label="get4Label('体重(kg)')"
                   name='bodyWeight'
                   prop='bodyWeight'
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['bodyLength']"
                   :label="get4Label('体长(cm)')"
                   name='bodyLength'
                   prop='bodyLength'
                   placeholder="请选择体长(cm)"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['bust']"
                   :label="get4Label('胸围(cm)')"
                   name='bust'
                   prop='bust'
                   placeholder="请选择胸围(cm)"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['pipeCircumference']"
                   :label="get4Label('管围(cm)')"
                   name='pipeCircumference'
                   prop='pipeCircumference'
                   placeholder="请选择管围(cm)"
                   inputMode="numeric"
                   :rules="[
...
const dataId = ref('')
const backRouteName = ref('BodyMeasurementList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/bodyMeasurement/bodyMeasurement/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/bodyMeasurement/bodyMeasurement/edit':'/bodyMeasurement/bodyMeasurement/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
