      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('任务状态')"
                 labelWidth="100px"
                 type="list"
                 name='renwustate'
                 dict=""
                v-model="myFormData['renwustate']"
              ></online-select>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('TestdictList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/testdict/testdict/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/testdict/testdict/edit':'/testdict/testdict/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
