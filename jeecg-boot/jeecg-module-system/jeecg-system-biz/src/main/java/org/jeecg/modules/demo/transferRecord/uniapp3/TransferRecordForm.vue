      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['transferRecord']"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fromShedPenId']"
                  :label="get4Label('原棚栏 ID')"
                  name='fromShedPenId'
                  prop='fromShedPenId'
                  placeholder="请选择原棚栏 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['toShedPenId']"
                  :label="get4Label('目标棚栏 ID')"
                  name='toShedPenId'
                  prop='toShedPenId'
                  placeholder="请选择目标棚栏 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('转群日期')"
                  labelWidth="100px"
                  type="date"
                  name='transferDate'
                  v-model:value="myFormData['transferDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('TransferRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/transferRecord/transferRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/transferRecord/transferRecord/edit':'/transferRecord/transferRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
