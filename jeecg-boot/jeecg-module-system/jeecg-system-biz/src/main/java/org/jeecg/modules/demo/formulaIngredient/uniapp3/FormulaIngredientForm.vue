        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('配方 ID')"
                  name='formulaId'
                  prop='formulaId'
                  placeholder="请选择配方 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('原料物资 ID')"
                  name='materialId'
                  prop='materialId'
                  placeholder="请选择原料物资 ID"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('FormulaIngredientList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/formulaIngredient/formulaIngredient/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/formulaIngredient/formulaIngredient/edit':'/formulaIngredient/formulaIngredient/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
