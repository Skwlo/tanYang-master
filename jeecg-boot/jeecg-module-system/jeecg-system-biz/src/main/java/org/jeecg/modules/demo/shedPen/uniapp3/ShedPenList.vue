...
<template>
  <PageLayout navTitle="场棚栏表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './ShedPenData';
defineOptions({
  name: 'ShedPenList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/shedPen/shedPen/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/shedPen/shedPen/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
