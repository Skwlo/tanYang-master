      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['saleRecordId']"
                  :label="get4Label('销售记录 ID')"
                  name='saleRecordId'
                  prop='saleRecordId'
                  placeholder="请选择销售记录 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('数量（通常为 1）')"
                   name='quantity'
                   prop='quantity'
                   placeholder="请选择数量（通常为 1）"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('单位（头 / 公斤）')"
                  name='unit'
                  prop='unit'
                  placeholder="请选择单位（头 / 公斤）"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('SaleRecordDetailList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/saleRecordDetail/saleRecordDetail/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/saleRecordDetail/saleRecordDetail/edit':'/saleRecordDetail/saleRecordDetail/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
