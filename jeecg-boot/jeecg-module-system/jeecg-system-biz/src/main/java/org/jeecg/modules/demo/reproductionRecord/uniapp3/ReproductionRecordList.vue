...
<template>
  <PageLayout navTitle="繁殖记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './ReproductionRecordData';
defineOptions({
  name: 'ReproductionRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/reproductionRecord/reproductionRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/reproductionRecord/reproductionRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
