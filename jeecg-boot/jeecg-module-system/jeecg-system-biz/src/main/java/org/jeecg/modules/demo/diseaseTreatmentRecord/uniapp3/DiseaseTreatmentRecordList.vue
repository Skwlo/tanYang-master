...
<template>
  <PageLayout navTitle="疾病治疗记录表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './DiseaseTreatmentRecordData';
defineOptions({
  name: 'DiseaseTreatmentRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/diseaseTreatmentRecord/diseaseTreatmentRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/diseaseTreatmentRecord/diseaseTreatmentRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
