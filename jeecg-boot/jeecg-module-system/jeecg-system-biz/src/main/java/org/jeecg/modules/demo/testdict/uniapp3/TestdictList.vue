...
<template>
  <PageLayout navTitle="字典"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './TestdictData';
defineOptions({
  name: 'TestdictList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/testdict/testdict/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/testdict/testdict/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
