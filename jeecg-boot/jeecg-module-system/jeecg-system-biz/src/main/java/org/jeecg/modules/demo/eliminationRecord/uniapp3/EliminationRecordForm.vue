      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('淘汰日期')"
                  labelWidth="100px"
                  type="date"
                  name='eliminationDate'
                  v-model:value="myFormData['eliminationDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('淘汰前类别')"
                 labelWidth="100px"
                 type="list"
                 name='beforeCategory'
                 dict=""
                v-model="myFormData['beforeCategory']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['eliminationReason']"
                  :label="get4Label('淘汰原因')"
                  name='eliminationReason'
                  prop='eliminationReason'
                  placeholder="请选择淘汰原因"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('EliminationRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/eliminationRecord/eliminationRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/eliminationRecord/eliminationRecord/edit':'/eliminationRecord/eliminationRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
