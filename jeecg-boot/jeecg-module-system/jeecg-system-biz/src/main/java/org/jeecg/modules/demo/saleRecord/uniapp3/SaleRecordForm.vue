      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('销售日期')"
                  labelWidth="100px"
                  type="date"
                  name='saleDate'
                  v-model:value="myFormData['saleDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['customerName']"
                  :label="get4Label('客户名称')"
                  name='customerName'
                  prop='customerName'
                  placeholder="请选择客户名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['customerAddress']"
                  :label="get4Label('客户地址')"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['customerPhone']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['salesman']"
                  :label="get4Label('销售人员')"
                  name='salesman'
                  prop='salesman'
                  placeholder="请选择销售人员"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['totalAmount']"
                   :label="get4Label('销售总额')"
                   name='totalAmount'
                   prop='totalAmount'
                   placeholder="请选择销售总额"
                   inputMode="numeric"
...
const dataId = ref('')
const backRouteName = ref('SaleRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/saleRecord/saleRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/saleRecord/saleRecord/edit':'/saleRecord/saleRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
