      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['parentId']"
                  :label="get4Label('父级 ID（场无父级，棚父级为场，栏父级为棚）')"
                  name='parentId'
                  prop='parentId'
                  placeholder="请选择父级 ID（场无父级，棚父级为场，栏父级为棚）"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('容量（可容纳畜只数量）')"
                   name='capacity'
                   prop='capacity'
                   placeholder="请选择容量（可容纳畜只数量）"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('棚栏 ID')"
                  name='shedPenId'
                  prop='shedPenId'
                  placeholder="请选择棚栏 ID"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('ShedPenList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/shedPen/shedPen/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/shedPen/shedPen/edit':'/shedPen/shedPen/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
