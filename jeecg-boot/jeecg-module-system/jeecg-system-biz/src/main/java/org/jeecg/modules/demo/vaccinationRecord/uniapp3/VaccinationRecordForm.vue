        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('畜只 ID（可选，群体免疫时为空）')"
                  name='livestockId'
                  prop='livestockId'
                  placeholder="请选择畜只 ID（可选，群体免疫时为空）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['vaccineId']"
                  :label="get4Label('疫苗 ID')"
                  name='vaccineId'
                  prop='vaccineId'
                  placeholder="请选择疫苗 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('免疫日期')"
                  labelWidth="100px"
                  type="date"
                  name='vaccinationDate'
                  v-model:value="myFormData['vaccinationDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['dosage']"
                   :label="get4Label('使用剂量')"
                   name='dosage'
                   prop='dosage'
                   placeholder="请选择使用剂量"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['injectionSite']"
                  :label="get4Label('注射部位')"
                  name='injectionSite'
                  prop='injectionSite'
                  placeholder="请选择注射部位"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['standard']"
                  :label="get4Label('合格指标')"
                  name='standard'
                  prop='standard'
                  placeholder="请选择合格指标"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('免疫范围（个体 / 群体）')"
                 labelWidth="100px"
                 type="list"
                 name='scope'
                 dict=""
                v-model="myFormData['scope']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('VaccinationRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/vaccinationRecord/vaccinationRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/vaccinationRecord/vaccinationRecord/edit':'/vaccinationRecord/vaccinationRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
