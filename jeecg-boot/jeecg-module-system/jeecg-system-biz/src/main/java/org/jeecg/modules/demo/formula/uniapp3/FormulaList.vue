...
<template>
  <PageLayout navTitle="配方表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './FormulaData';
defineOptions({
  name: 'FormulaList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/formula/formula/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/formula/formula/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
