        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('幼畜 ID')"
                  name='livestockId'
                  prop='livestockId'
                  placeholder="请选择幼畜 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('断奶日期')"
                  labelWidth="100px"
                  type="date"
                  name='weaningDate'
                  v-model:value="myFormData['weaningDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['weaningAge']"
                   :label="get4Label('断奶日龄')"
                   name='weaningAge'
                   prop='weaningAge'
                   placeholder="请选择断奶日龄"
                   inputMode="numeric"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['weaningWeight']"
                   :label="get4Label('断奶重量 (kg)')"
                   name='weaningWeight'
                   prop='weaningWeight'
                   placeholder="请选择断奶重量 (kg)"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['newShedPenId']"
                  :label="get4Label('新棚栏 ID')"
                  name='newShedPenId'
                  prop='newShedPenId'
                  placeholder="请选择新棚栏 ID"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('WeaningRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/weaningRecord/weaningRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/weaningRecord/weaningRecord/edit':'/weaningRecord/weaningRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
