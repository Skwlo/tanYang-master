...
navigationBarTitleText: '畜只类型变更记录表',
},
}
</route>
...
const dataId = ref('')
const backRouteName = ref('LivestockTypeChangeList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/livestockTypeChange/livestockTypeChange/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/livestockTypeChange/livestockTypeChange/edit':'/livestockTypeChange/livestockTypeChange/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
