      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['currentQuantity']"
                   :label="get4Label('当前库存数量')"
                   name='currentQuantity'
                   prop='currentQuantity'
                   placeholder="请选择当前库存数量"
                   inputMode="numeric"
                   :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('最后入库日期')"
                  labelWidth="100px"
                  type="date"
                  name='lastInDate'
                  v-model:value="myFormData['lastInDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('最后出库日期')"
                  labelWidth="100px"
                  type="date"
                  name='lastOutDate'
                  v-model:value="myFormData['lastOutDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('InventoryList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/inventory/inventory/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/inventory/inventory/edit':'/inventory/inventory/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
