      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('喂食日期')"
                  labelWidth="100px"
                  type="date"
                  name='feedDate'
                  v-model:value="myFormData['feedDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('喂食棚栏 ID')"
                  name='shedPenId'
                  prop='shedPenId'
                  placeholder="请选择喂食棚栏 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['formulaId']"
                  :label="get4Label('使用配方 ID')"
                  name='formulaId'
                  prop='formulaId'
                  placeholder="请选择使用配方 ID"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('投料类别（精料 / 粗料）')"
                 labelWidth="100px"
                 type="list"
                 name='feedType'
                 dict=""
                v-model="myFormData['feedType']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('喂食数量 (kg)')"
                   name='quantity'
                   prop='quantity'
                   placeholder="请选择喂食数量 (kg)"
                   inputMode="numeric"
                   :rules="[
...
const dataId = ref('')
const backRouteName = ref('FeedingRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/feedingRecord/feedingRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/feedingRecord/feedingRecord/edit':'/feedingRecord/feedingRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
