...
<script setup lang="ts">
  ...
  // 页面挂载后获取后端统计
  ///livestock/livestock/statistics
  onMounted(() => {
    console.log('mouted方法');
    cowcowTotal().then((data) => {
      console.log(data, '11111111111111111111111111');
      console.log(data.normalCount);
      totalCount.value = data.totalCount;
      todayCount.value = data.todayCount;
      normalCount.value = data.normalCount;
    ...
    // axios
    //   .get('/jeecg-boot/livestock/livestock/statistics')
    //   .then((res) => {
    //     console.log('后端返回了');
    //     console.log('🐮 后端返回数据:', res);

    // const result = res.data.result;
    // console.log('🐮 解构后的 result:', result);
    //
    // totalCount.value = result.totalCount;
    //
    // normalCount.value = result.normalCount;
    // todayCount.value = result.todayCount;
    // })
    // .catch((err) => {
    //   console.error('🐮 获取牛牛统计失败:', err);
    // });
